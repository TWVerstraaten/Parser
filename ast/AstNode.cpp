...
namespace ast {
    ...
    u_ptr_AstNode AstNode::makeZeroNode() {
        return makeNumber(gen::Number(0ll));
    }

    u_ptr_AstNode AstNode::makeOneNode() {
        return makeNumber(gen::Number(1ll));
    }

    u_ptr_AstNode AstNode::makeNumber(const gen::Number& val) {
        return std::make_unique<AstNodeNumber>(val);
    }

    gen::Number AstNode::getNumber() const {
